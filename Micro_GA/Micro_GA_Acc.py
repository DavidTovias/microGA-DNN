...
# 
# 
# THIS FILE IS FOR OBJECTIVE FUNCTION 'acc' 
##################
...
class MicroGA_ACC(IMetaheuristic):
    ...
    # if equal fitness, percentage of neurons as a 2nd criterion
    def selection(self):
        npop = self.params['npop']
        ...
        for i in range(npop-1):
            if round(self.POP[ind1[i]].fitness,2) > round(self.POP[ind1[i+1]].fitness,2):
                self.PARENTS[i] = copy.deepcopy(self.POP[ind1[i]])
            
            elif round(self.POP[ind1[i]].fitness,2) < round(self.POP[ind1[i+1]].fitness,2):
                self.PARENTS[i] = copy.deepcopy(self.POP[ind1[i+1]])
            
            elif self.POP[ind1[i]].pneurons < self.POP[ind1[i+1]].pneurons:
                print("pneruons")
                self.PARENTS[i] = copy.deepcopy(self.POP[ind1[i]])
            else:
                self.PARENTS[i] = copy.deepcopy(self.POP[ind1[i+1]])
        ...
        # compare the last one with first one
        if round(self.POP[ind1[-1]].fitness, 2) > round(self.POP[ind1[0]].fitness, 2):
            self.PARENTS[-1] = copy.deepcopy(self.POP[ind1[-1]])
        elif round(self.POP[ind1[-1]].fitness, 2) < round(self.POP[ind1[0]].fitness, 2):
            self.PARENTS[-1] = copy.deepcopy(self.POP[ind1[0]])
        
        elif self.POP[ind1[-1]].pneurons < self.POP[ind1[0]].pneurons:
            self.PARENTS[-1] = copy.deepcopy(self.POP[ind1[-1]])
            print("pneruons")
        else:
            self.PARENTS[-1] = copy.deepcopy(self.POP[ind1[0]])
    ...
    def crossover(self):
        npop = self.params['npop']
        bindim = self.POP[0].params['bindim']
        pc = self.params['pc']
    ...
    def select_best(self, pop:str):
        ...
        if pop == 'POP':
            ...
            for i in range(npop):
                if i != idx_best:
                    if self.POP[i].pneurons < self.POP[idx_best].pneurons and (round(self.POP[i].fitness, 2) == round(self.POP[idx_best].fitness,2) ):
                        the_best = i
                        print("Entro a the best", pop)
            print("the best: ", the_best)
            self.best_ind = self.POP[the_best]
            

        elif pop == 'OFFSPRING':
            # Max fitness of OFFSPRING
            for i in range(npop):
            ...
            for i in range(npop):
                if i != idx_best:
                    if self.OFFSPRING[i].pneurons < self.OFFSPRING[idx_best].pneurons and (round(self.OFFSPRING[i].fitness,2) == round(self.OFFSPRING[idx_best].fitness,2) ):
                        the_best = i
                        print("Entro a the best", pop)
            
            # Update the best solution
            if round(self.OFFSPRING[the_best].fitness, 2) > round(self.best_ind.fitness, 2):
                self.best_ind = self.OFFSPRING[the_best]
            
            elif round(self.OFFSPRING[the_best].fitness, 2) < round(self.best_ind.fitness, 2):
                # Apply elitism
                ind = np.random.randint(0,npop)
                self.OFFSPRING[ind] = self.best_ind
            
            elif self.OFFSPRING[the_best].pneurons < self.best_ind.pneurons:
                self.best_ind = self.OFFSPRING[the_best]
            ...
            for i in range(npop):
                if i != idx_best:
                    if self.AUX_POP[i].pneurons < self.AUX_POP[idx_best].pneurons and (round(self.AUX_POP[i].fitness,2) == round(self.AUX_POP[idx_best].fitness,2) ):
                        the_best = i
                        print("Entro a the best", pop)
            
            # Update the best solution
            if round(self.AUX_POP[the_best].fitness, 2) > round(self.best_ind.fitness, 2):
                self.best_ind = self.AUX_POP[the_best]

            elif round(self.AUX_POP[the_best].fitness, 2) < round(self.best_ind.fitness, 2):
                # Apply elitism
                ind = np.random.randint(0,npop)
                self.AUX_POP[ind] = self.best_ind
            
            elif self.AUX_POP[the_best].pneurons < self.best_ind.pneurons:
                self.best_ind = self.AUX_POP[the_best]
